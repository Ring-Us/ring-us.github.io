...
const StatusTabs: React.FC<StatusTabsProps> = ({ selectedStatus, onStatusChange }) => {
  const statuses: Array<"미확정" | "확정" | "완료"> = ["미확정", "확정", "완료"];
    ...
    <div className="flex h-[40px] mt-[5px]">
      {statuses.map(status => (
        <div
          ...
          className={`w-1/3 text-[16px] font-bold flex transition-all border-gray-3 border-b-[1px] justify-center items-start cursor-pointer 
